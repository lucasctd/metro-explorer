...
export default class ExplorerComponent extends Vue {
	...
	mounted() {
        ...
        document.getElementById(this.rootId).addEventListener("movingToChild", (e) => {
            console.log("movingToChild");
		});
	}
	...
	private findFolderChildren(parentId: number): Array<File> {
		...
		this.files.forEach(f => {
			if(f.parent !== undefined && f.parent.id === parentId && f.dir === true) {
				children.push(f);
	...
	getLeftPos(file: File, numGridX: number, fileField?: number): number {
        return FileUtils.getLeftPos(file, numGridX, fileField);
	}

	getTopPos(file: File, numGridX: number, fileField?: number): number {
        return FileUtils.getTopPos(file, numGridX, fileField);
	}
    ...
    set currentDir(file: File){
        this.$parent.$data['currentDir'] = file;
    }
}